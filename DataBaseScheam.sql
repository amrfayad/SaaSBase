...
CREATE TABLE IF NOT EXISTS `saasBase`.`teams` (
  ...
  `subscribtions_subscr_id` INT NULL,
  PRIMARY KEY (`team_id`),
  INDEX `fk_teams_users1_idx` (`users_user_id` ASC),
  INDEX `fk_teams_subscribtions1_idx` (`subscribtions_subscr_id` ASC),
  CONSTRAINT `fk_teams_users1`
    FOREIGN KEY (`users_user_id`)
    REFERENCES `saasBase`.`users` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_teams_subscribtions1`
    FOREIGN KEY (`subscribtions_subscr_id`)
    REFERENCES `saasBase`.`subscribtions` (`subscr_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
